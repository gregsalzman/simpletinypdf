...
namespace SimpleTinyPDF.Tests
{
    public class TextRenderingTests
    {
        ...
        public void DrawText_RendersVisibleText()
        {
            ...
            TestHelper.AddDescription(page, "Verify: 'Hello, PDF!' renders in Helvetica 24pt");
            page.DrawText("Hello World", 50, 50, PdfFont.Helvetica, 24);
            var bytes = doc.ToArray();

            TestHelper.SavePdf(bytes, "Text/hello-world-helvetica-24pt");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/hello-world-helvetica-24pt");
            // Text at (50,50) with fontSize 24. At 150 DPI: x~104px, y~104-154px
            int textY = TestHelper.PtToPx(50);
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(250), textY, textY + TestHelper.PtToPx(24)),
        ...
        public void DrawText_AllFonts_RenderWithoutError()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: all 14 built-in PDF fonts render correctly");
            float y = 30;
            var fonts = new[]
            {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/all-14-builtin-fonts");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/all-14-builtin-fonts");
            // Verify each font line has visible text at its expected Y position
            float checkY = 30;
            for (int i = 0; i < fonts.Length; i++)
            {
                int py = TestHelper.PtToPx(checkY);
                Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(300), py, py + TestHelper.PtToPx(14)),
        ...
        public void DrawText_ColoredText_RendersInColor()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: text renders in red color");
            // Draw a large red block of text
            page.DrawText("RED TEXT", 50, 50, PdfFont.HelveticaBold, 48, PdfColor.Red);
            var bytes = doc.ToArray();

            TestHelper.SavePdf(bytes, "Text/red-colored-text");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/red-colored-text");
        ...
        public void DrawText_CenterAlignment_IsCenteredRelativeToX()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: text is horizontally centered on the page");
            // Draw centered text at the middle of the page
            page.DrawText("Center", page.Width / 2, 50, PdfFont.Helvetica, 24,
                alignment: TextAlignment.Center);
            var bytes = doc.ToArray();

            TestHelper.SavePdf(bytes, "Text/center-aligned-text");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/center-aligned-text");
            int midX = bitmap.Width / 2;
            int textY = TestHelper.PtToPx(50);
            // Text should be visible near the horizontal center
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, midX - TestHelper.PtToPx(50), midX + TestHelper.PtToPx(50), textY, textY + TestHelper.PtToPx(24)),
            ...
            Assert.False(TestHelper.HasDarkPixelsInRegion(bitmap, 5, TestHelper.PtToPx(30), textY, textY + TestHelper.PtToPx(24)),
        ...
        public void DrawText_RightAlignment_Works()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: text is right-aligned on the page");
            page.DrawText("Right", page.Width - 50, 50, PdfFont.Helvetica, 24,
                alignment: TextAlignment.Right);
            var bytes = doc.ToArray();

            TestHelper.SavePdf(bytes, "Text/right-aligned-text");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/right-aligned-text");
            int rightEdge = TestHelper.PtToPx(page.Width - 50);
            int textY = TestHelper.PtToPx(50);
            // Text should be visible near the right side (text ends at x=page.Width-50)
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, rightEdge - TestHelper.PtToPx(80), rightEdge, textY, textY + TestHelper.PtToPx(24)),
        ...
        public void DrawText_WrapsText()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: long text wraps within textbox bounds");
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/textbox-word-wrapped");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/textbox-word-wrapped");
            ...
            for (int line = 0; line < linesRendered; line++)
            {
                ...
                int py = TestHelper.PtToPx(lineY);
                Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(250), py, py + TestHelper.PtToPx(12)),
        ...
        public void DrawText_ReturnsCorrectY_ForChaining()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: text overflows from first textbox into chained second textbox");
            float y = 50;
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/textbox-overflow-chain");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/textbox-overflow-chain");
            // First paragraph should have text near Y=50
            int py1 = TestHelper.PtToPx(50);
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(300), py1, py1 + TestHelper.PtToPx(14)),
                "Expected visible text for first paragraph");
            // Second paragraph should have text at its starting position
            int py2 = TestHelper.PtToPx(secondStart);
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(300), py2, py2 + TestHelper.PtToPx(14)),
        ...
        public void DrawText_RightAlignment_MultiLineAligns()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: right-aligned text wraps correctly across multiple lines");
            float boxX = 100;
            float boxY = 50;
            float boxWidth = 300;
            float fontSize = 14f;
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/textbox-right-aligned-multiline");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/textbox-right-aligned-multiline");
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                int yMin = TestHelper.PtToPx(lineY);
                int yMax = yMin + TestHelper.PtToPx(fontSize);
                yMax = System.Math.Min(yMax, bitmap.Height - 1);

                rightmostDark[i] = -1;
                for (int x = System.Math.Min(rightEdgePx + TestHelper.PtToPx(10), bitmap.Width - 1); x >= TestHelper.PtToPx(boxX); x--)
                {
            ...
            int tolerancePx = TestHelper.PtToPx(4); // allow ~4pt of rasterisation slop
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                for (int x = TestHelper.PtToPx(boxX); x <= rightEdgePx && x < bitmap.Width; x++)
                {
                    for (int y = yMin; y <= yMax; y++)
                    {
                        var p = bitmap.GetPixel(x, y);
        ...
        public void DrawText_Underline_RendersLineBelow()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: underlined text has visible underline");
            ...
            TestHelper.SavePdf(bytes, "Text/underlined-text");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/underlined-text");
            ...
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(200),
            ...
            Assert.False(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(200),
                TestHelper.PtToPx(noUlTop), TestHelper.PtToPx(noUlBottom)),
        ...
        public void DrawText_Underline_RendersOnAllLines()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: underlined text wraps correctly in textbox");
            float endY = page.DrawText(
                "This is text that should wrap and each line should be underlined.",
                50, 50, PdfFont.Helvetica, 14, underline: true, width: 200);

            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/textbox-underlined-wrapped");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/textbox-underlined-wrapped");
        ...
        public void DrawText_DifferentSizes_ScaleCorrectly()
        {
            var doc = new PdfDocument();
            var page = doc.AddPage(PageSize.A4);
            TestHelper.AddDescription(page, "Verify: text renders at multiple font sizes (8-48pt)");
            page.DrawText("Small", 50, 50, fontSize: 8);
            page.DrawText("Medium", 50, 70, fontSize: 16);
            page.DrawText("Large", 50, 100, fontSize: 32);

            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "Text/multiple-font-sizes");
            var bitmap = TestHelper.RasterizePage(bytes, "Text/multiple-font-sizes");

            // Verify each text size is visible at its expected position
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(150), TestHelper.PtToPx(50), TestHelper.PtToPx(58)),
                "Expected visible 'Small' text at Y=50");
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(200), TestHelper.PtToPx(70), TestHelper.PtToPx(86)),
                "Expected visible 'Medium' text at Y=70");
            Assert.True(TestHelper.HasDarkPixelsInRegion(bitmap, TestHelper.PtToPx(50), TestHelper.PtToPx(250), TestHelper.PtToPx(100), TestHelper.PtToPx(132)),
            ...
            int smallY = TestHelper.PtToPx(54), largeY = TestHelper.PtToPx(116);
            for (int x = TestHelper.PtToPx(50); x < TestHelper.PtToPx(300) && x < bitmap.Width; x++)
            {
