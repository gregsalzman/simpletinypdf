...
namespace SimpleTinyPDF.Tests
{
    public class ImageScaleModeTests
    {
        ...
        public void Stretch_FillsEntireArea()
        {
            ...
            TestHelper.SavePdf(bytes, "scale_stretch");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_stretch");

            // All four quadrants visible; corners should not be white
            var tl = bitmap.GetPixel(PtToPx(120), PtToPx(120));
            Assert.True(tl.Red > 150, $"Stretch TL should be red, got ({tl.Red},{tl.Green},{tl.Blue})");

            var br = bitmap.GetPixel(PtToPx(280), PtToPx(280));
        ...
        public void Fit_LandscapeImage_InSquareBox_HasVerticalWhitespace()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fit_landscape");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fit_landscape");

            // Center of box should be the image (red)
            var center = bitmap.GetPixel(PtToPx(200), PtToPx(200));
            ...
            var topBand = bitmap.GetPixel(PtToPx(200), PtToPx(110));
            ...
            var bottomBand = bitmap.GetPixel(PtToPx(200), PtToPx(290));
        ...
        public void Fit_PortraitImage_InSquareBox_HasHorizontalWhitespace()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fit_portrait");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fit_portrait");
            ...
            var leftBand = bitmap.GetPixel(PtToPx(110), PtToPx(200));
            ...
            var rightBand = bitmap.GetPixel(PtToPx(290), PtToPx(200));
        ...
        public void Fill_LandscapeImage_InSquareBox_CropsTopAndBottom()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fill_landscape");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fill_landscape");

            // The box at (100,100)-(300,300) should be completely filled (no white)
            var topLeft = bitmap.GetPixel(PtToPx(110), PtToPx(110));
            ...
            var bottomRight = bitmap.GetPixel(PtToPx(290), PtToPx(290));
            ...
            var outside = bitmap.GetPixel(PtToPx(90), PtToPx(200));
        ...
        public void Fill_PortraitImage_InSquareBox_CropsLeftAndRight()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fill_portrait");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fill_portrait");

            // The box at (100,200)-(300,400) should be completely filled
            var topLeft = bitmap.GetPixel(PtToPx(110), PtToPx(210));
            Assert.True(!(topLeft.Red > 245 && topLeft.Green > 245 && topLeft.Blue > 245),
                $"Fill TL should not be white, got ({topLeft.Red},{topLeft.Green},{topLeft.Blue})");

            var bottomRight = bitmap.GetPixel(PtToPx(290), PtToPx(390));
            Assert.True(!(bottomRight.Red > 245 && bottomRight.Green > 245 && bottomRight.Blue > 245),
            ...
            var above = bitmap.GetPixel(PtToPx(200), PtToPx(190));
        ...
        public void Fill_ClipsOverflow_OutsideBoxIsWhite()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fill_clip");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fill_clip");

            // Inside the box should be red (the test JPEG is solid red)
            var inside = bitmap.GetPixel(PtToPx(250), PtToPx(250));
            ...
            var outsideLeft = bitmap.GetPixel(PtToPx(190), PtToPx(250));
            ...
            var outsideRight = bitmap.GetPixel(PtToPx(310), PtToPx(250));
        ...
        public void Fit_SquareImage_InSquareBox_NoWhitespace()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_fit_square");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_fit_square");

            // All corners of the box should have image content (red)
            var tl = bitmap.GetPixel(PtToPx(110), PtToPx(110));
            Assert.True(tl.Red > 150, $"Fit square TL should be red, got ({tl.Red},{tl.Green},{tl.Blue})");

            var br = bitmap.GetPixel(PtToPx(290), PtToPx(290));
        ...
        public void DefaultScaleMode_IsStretch()
        {
            ...
            var bytes = doc.ToArray();
            TestHelper.SavePdf(bytes, "scale_default");
            var bitmap = TestHelper.RasterizePage(bytes, "scale_default");
