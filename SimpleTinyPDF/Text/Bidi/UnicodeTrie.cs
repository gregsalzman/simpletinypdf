    ...
    internal class UnicodeTrie
    {
        ...
        public uint Get(int codePoint)
        {
            ...
            if ((codePoint < 0xd800) || ((codePoint > 0xdbff) && (codePoint <= 0xffff)))
            {
                ...
                index = (_data[codePoint >> UnicodeTrieConstants.SHIFT_2] << UnicodeTrieConstants.INDEX_SHIFT) + (codePoint & UnicodeTrieConstants.DATA_MASK);
            ...
            if (codePoint <= 0xffff)
            {
                ...
                index = (_data[UnicodeTrieConstants.LSCP_INDEX_2_OFFSET + ((codePoint - 0xd800) >> UnicodeTrieConstants.SHIFT_2)] << UnicodeTrieConstants.INDEX_SHIFT) + (codePoint & UnicodeTrieConstants.DATA_MASK);
                return (uint)_data[index];
            }
            ...
            if (codePoint < _highStart)
            {
                ...
                index = _data[(UnicodeTrieConstants.INDEX_1_OFFSET - UnicodeTrieConstants.OMITTED_BMP_INDEX_1_LENGTH) + (codePoint >> UnicodeTrieConstants.SHIFT_1)];
                index = _data[index + ((codePoint >> UnicodeTrieConstants.SHIFT_2) & UnicodeTrieConstants.INDEX_2_MASK)];
                index = (index << UnicodeTrieConstants.INDEX_SHIFT) + (codePoint & UnicodeTrieConstants.DATA_MASK);
                return (uint)_data[index];
            }

            return (uint)_data[_data.Length - UnicodeTrieConstants.DATA_GRANULARITY];
        }
    }
}
